...
import { Answer, Dialog, ResponseType, User } from '@/types/types';
...
function Chat({params}) {
    ...
    useEffect(() => {
        async function getConvo() {
            const {data} = await supabase.from('dialogs').select('*').eq('conversation_id', params.chatId);
                ...
                <div className='w-full h-full flex flex-col justify-between'>
                    <div className='w-full h-full overflow-auto px-2 flex flex-col'>
                        <div className='h-5/6 w-full overflow-auto my-8'>
                            <InfiniteScroll dataLength={response.length} next={() => { }} hasMore={false} loader={<PulseLoader color={'#000000'} size={10} aria-label="Loading Spinner" data-testid="loader" />} className='w-full xl:px-44'>
                                {conversation.map(convo => <Response answer={convo.answer} question={convo.question} sources={convo.links} userId={user.id} />)}
                                {!response || response.length === 0 && isQuestion === false ? null : response.map(res => <Response answer={res.answer} question={res.question} userId={user.id} sources={res.links} />)}
